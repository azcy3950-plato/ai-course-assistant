...
export default function KnowledgeGraphPanel(p: Props) {
  ...
  useEffect(() => {
    ...
    const onPointerDown = (e: PointerEvent) => { e.preventDefault(); (e.target as Element).setPointerCapture(e.pointerId); dragRef.current = { startX: e.clientX, startY: e.clientY, tx: viewRef.current.tx, ty: viewRef.current.ty, active: true, moved: false }; };
