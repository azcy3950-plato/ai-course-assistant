...
export default function GuidedPage() {
  ...
  return <div className="h-[calc(100vh-3.5rem)] min-h-[620px] bg-slate-100 p-3 md:p-4"><div className="mx-auto flex h-full max-w-[1600px] flex-col overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm"><header className="flex shrink-0 items-center justify-between border-b px-4 py-3"><div><h1 className="text-base font-bold text-slate-900">引导学习工作台</h1><p className="text-xs text-slate-500">苏格拉底式引导 · 三轮追问 · 四级提示</p>{socraticActive && <div className="mt-1.5 flex items-center gap-2 text-[11px]">{[1, 2, 3].map((r) => <span key={r} className={`rounded-full px-2 py-0.5 ${r === turn ? "bg-blue-600 text-white" : r < turn ? "bg-emerald-100 text-emerald-700" : "bg-slate-100 text-slate-400"}`}>{r === 1 ? "① 识别问题" : r === 2 ? "② 建立联系" : "③ 形成答案"}</span>)}<span className="text-slate-400">提示 {hintLevel}/{MAX_HINTS}</span></div>}</div><div className="hidden gap-2 text-xs text-slate-500 md:flex"><span className="rounded-full bg-blue-50 px-3 py-1 text-blue-700">累计 {cumulative.nodes.length} 个概念</span><span className="rounded-full bg-emerald-50 px-3 py-1 text-emerald-700">稳定布局</span></div></header><div className="flex border-b md:hidden"><button onClick={() => setMobileTab("graph")} className={`flex-1 py-2 text-sm ${mobileTab === "graph" ? "border-b-2 border-blue-600 text-blue-700" : "text-slate-500"}`}>知识图谱</button><button onClick={() => setMobileTab("chat")} className={`flex-1 py-2 text-sm ${mobileTab === "chat" ? "border-b-2 border-blue-600 text-blue-700" : "text-slate-500"}`}>AI 对话</button></div><div ref={workspace} className="min-h-0 flex-1 md:grid" style={{ gridTemplateColumns: graphCollapsed ? "0 1fr" : `${ratio}% 1fr` }}>
    ...
    <section className={`flex min-w-0 flex-col bg-slate-50 ${mobileTab === "chat" ? "flex" : "hidden md:flex"}`}>{graphCollapsed && <button onClick={() => setGraphCollapsed(false)} className="m-3 self-start rounded-lg border bg-white px-3 py-1.5 text-xs text-blue-700">展开知识图谱</button>}<div className="min-h-0 flex-1 overflow-y-auto p-4 md:p-6"><div className="mx-auto max-w-3xl space-y-4">{messages.length === 0 && <div className="py-10 text-center md:py-20"><div className="mx-auto mb-4 flex h-14 w-14 items-center justify-center rounded-2xl bg-blue-100 text-2xl">✦</div><h2 className="text-lg font-bold text-slate-900">从一个问题开始，我们一起思考</h2><p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">我不会直接告诉你答案——通过最多三轮追问和四级提示，引导你自己找到答案，并把相关概念呈现在左侧图谱上。</p><div className="mt-6 flex flex-wrap justify-center gap-2">{suggested.map((q) => <button key={q} onClick={() => send(q)} className="rounded-full border bg-white px-3 py-2 text-xs text-slate-600 hover:border-blue-400 hover:text-blue-700">{q}</button>)}</div></div>}{messages.map((m) => <article key={m.id} className={`flex gap-3 ${m.role === "user" ? "justify-end" : ""}`}><div className={`max-w-[min(88%,720px)] rounded-2xl px-4 py-3 text-sm leading-7 shadow-sm ${m.role === "user" ? "bg-blue-600 text-white" : m.kind === "hint" ? "border border-amber-200 bg-amber-50 text-amber-900" : m.kind === "final" ? "border border-emerald-200 bg-emerald-50 text-emerald-900" : m.kind === "info" ? "border border-slate-200 bg-slate-50 text-slate-500" : "border bg-white text-slate-700"}`}><div>{m.content}</div>{m.pending && <div className="mt-2 text-xs text-blue-500">● ● ●</div>}{m.role === "assistant" && m.concepts?.length ? <div className="mt-3 border-t pt-3"><div className="mb-2 text-xs font-semibold text-slate-500">本轮核心概念</div><div className="flex flex-wrap gap-1.5">{m.concepts.slice(0, 6).map((n) => <button key={n.id} onClick={() => { setSelected(n); setFocusIds([n.id]); }} className="rounded-full bg-blue-50 px-2.5 py-1 text-xs text-blue-700 hover:bg-blue-100">{n.name}</button>)}</div><button onClick={() => graphForMessage(m)} className="mt-3 text-xs text-blue-600 hover:underline">查看本轮知识路径 →</button></div> : null}</div></article>)}</div></div><div className="shrink-0 border-t bg-white p-3 md:p-4"><div className="mx-auto flex max-w-3xl items-end gap-2"><textarea value={input} onChange={(e) => setInput(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); send(); } }} placeholder={socraticActive ? "输入你的思考/回答（也可直接输入新问题重新开始）…" : "输入你想学习的问题…（Enter 发送，Shift+Enter 换行）"} rows={2} disabled={loading} className="min-h-[52px] flex-1 resize-none rounded-xl border px-4 py-3 text-sm outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-100 disabled:bg-slate-100" />{socraticActive && <button onClick={requestHint} disabled={loading || hintLevel >= MAX_HINTS} title={hintLevel >= MAX_HINTS ? "提示已用满" : "获取逐级提示"} className="shrink-0 rounded-xl border border-amber-300 bg-amber-50 px-3 py-3 text-xs font-medium text-amber-700 hover:bg-amber-100 disabled:opacity-40">💡 提示 {hintLevel}/{MAX_HINTS}</button>}{socraticActive && <button onClick={resetSocratic} disabled={loading} title="结束本轮引导" className="shrink-0 rounded-xl border px-3 py-3 text-xs text-slate-500 hover:bg-slate-50 disabled:opacity-40">结束</button>}<button onClick={() => send()} disabled={loading || !input.trim()} className="rounded-xl bg-blue-600 px-5 py-3 text-sm font-medium text-white disabled:opacity-40">{loading ? "生成中" : socraticActive ? "回答" : "发送"}</button></div><div className="mx-auto mt-2 flex max-w-3xl items-center justify-between text-[10px] text-slate-400"><span>{socraticActive ? `苏格拉底引导中 · 第 ${turn}/${MAX_TURNS} 轮追问 · 提示 ${hintLevel}/${MAX_HINTS}` : focusIds.length ? `当前聚焦：${focusIds.map((id) => nodeForId(id)?.name).filter(Boolean).join("、")}` : "提出一个问题，我会用追问引导你思考"}</span>{loading && <button onClick={() => abortRef.current?.abort()} className="text-blue-600">停止生成</button>}</div></div></section>
  </div></div></div>;
}
