...
import LegacyLearningTools, { type LearningTool } from "@/components/guided/LegacyLearningTools";
...
export default function GuidedPage() {
  const [learningTool, setLearningTool] = useState<LearningTool | null>(null);
  const [collectRequest, setCollectRequest] = useState(0);
  const [compareRequest, setCompareRequest] = useState(0);
  const [conceptWalk, setConceptWalk] = useState<{ ids: string[]; index: number; playing: boolean } | null>(null);
  const { state } = useApp();
  ...
  const graph = mode === "cumulative" && cumulative.nodes.length ? cumulative : fullGraph; // 完整图谱模式：始终显示全部节点，焦点只影响高亮与中心
  ...
  const askAbout = (node: KnowledgeNode) => { setSelected(node); setInput(`请解释“${node.name}”，并说明它在${node.chapter}中的作用与适用条件。`); setMobileTab("chat"); };
  ...
  const focusToolNode = (node: KnowledgeNode) => {
    setSelected(null);
    setMode("current");
    if (networkIdOf(node.id) !== activeNetwork) switchNetwork(networkIdOf(node.id), node.id);
    else setFocusIds([node.id]);
  };
  const walkFocus = useRef(focusToolNode); walkFocus.current = focusToolNode;
  useEffect(() => {
    if (!conceptWalk?.playing) return;
    const node = nodeForId(conceptWalk.ids[conceptWalk.index]);
    if (node) walkFocus.current(node);
    const timer = window.setTimeout(() => setConceptWalk(old => !old ? null : old.index >= old.ids.length - 1 ? { ...old, playing: false } : { ...old, index: old.index + 1 }), 1800);
    return () => window.clearTimeout(timer);
  }, [conceptWalk, nodeForId]);
  ...
  setSelected(null);
}} className="rounded-xl border border-slate-200 px-2 py-2 text-xs transition-all duration-200 hover:-translate-y-0.5 hover:shadow-sm">加入复习</button><button onClick={() => {
  // 接通真实掌握标记：记录 GUIDED_COMPLETED 学习事件（此前是空操作，只关闭面板）
  fetch("/api/learning-events", {
    method: "POST",
    headers: { "Content-Type": "application/json", Authorization: `Bearer ${getAuthToken()}` },
    body: JSON.stringify({ type: "GUIDED_COMPLETED", title: `标记掌握：${selected.name}`, summary: `在引导学习中标记掌握知识点「${selected.name}」`, refType: "node", refId: selected.id }),
  }).catch(() => {});
  setSelected(null);
}} className="rounded-xl border border-slate-200 px-2 py-2 text-xs transition-all duration-200 hover:-translate-y-0.5 hover:shadow-sm">标记已掌握</button></div><details className="legacy-more-actions"><summary>更多互动：比较、演示、练习与整理</summary><div><button onClick={() => { setCompareRequest(v => v + 1); setLearningTool("compare"); }}>加入比较</button><button onClick={() => setLearningTool("lab")}>操作原理</button><button onClick={() => setLearningTool("practice")}>动手练习</button><button onClick={() => { setCollectRequest(v => v + 1); setLearningTool("notes"); }}>收入整理区</button><button onClick={() => setLearningTool("replay")}>学习回放</button></div></details></aside>}</section>
