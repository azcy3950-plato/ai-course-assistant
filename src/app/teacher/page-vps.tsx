...
export default function TeacherPage() {
  ...
  const handleFiles = useCallback(async (fileList: FileList | null) => {
    ...
    try {
      ...
      // 3) Save metadata to local PostgreSQL
  ...
  const handleDelete = useCallback(async (doc: DocFile) => {
    if (!confirm("确定删除？")) return;
    try {
      await fetch(API_STORAGE, { method: "DELETE", headers: await getAuthHeaders(), body: JSON.stringify({ fileKey: doc.r2_key }) });
      await fetch(API_DOCS, { method: "DELETE", headers: await getAuthHeaders(), body: JSON.stringify({ id: doc.id, key: doc.r2_key }) });
    ...
    <div className="max-w-6xl mx-auto px-4 py-6">
      ...
      <div className="flex gap-1 mb-6 bg-gray-100 rounded-xl p-1 w-fit">
        {[{ k: "upload" as TabKey, l: "资料上传", i: "📤" }, { k: "knowledge" as TabKey, l: "知识库管理", i: "📚" }, { k: "students" as TabKey, l: "学生统计", i: "📊" }].map(t => (
          <button key={t.k} onClick={() => setActiveTab(t.k)} className={"px-4 py-2 rounded-lg text-sm font-medium transition-colors " + (activeTab === t.k ? "bg-white text-[var(--color-text)] shadow-sm" : "text-[var(--color-text-secondary)] hover:text-[var(--color-text)]")}>{t.i} {t.l}</button>
        ))}
      </div>
      ...
      {activeTab === "upload" && (
        <div>
          <div onDragOver={e => { e.preventDefault(); setDragOver(true); }} onDragLeave={() => setDragOver(false)} onDrop={e => { e.preventDefault(); setDragOver(false); handleFiles(e.dataTransfer.files); }}
            ...
            <p className="text-sm text-[var(--color-text-secondary)] mb-4">支持 PDF、PPTX、DOCX、图片等，单文件最大 200MB</p>
            <button onClick={() => fileInputRef.current?.click()} disabled={uploading} className="inline-flex items-center gap-1.5 px-5 py-2.5 bg-[var(--color-primary)] text-white rounded-lg text-sm font-medium hover:opacity-90 disabled:opacity-50">+ {uploading ? "上传中..." : "选择文件"}</button>
            <input ref={fileInputRef} type="file" accept=".pdf,.ppt,.pptx,.doc,.docx,.png,.jpg,.jpeg,.gif,.webp" className="hidden" onChange={e => handleFiles(e.target.files)} />
          </div>
          ...
          <div className="bg-white rounded-xl border border-[var(--color-border)]">
            <div className="px-5 py-3 border-b border-[var(--color-border)] flex items-center justify-between"><h3 className="text-sm font-bold">已上传文件</h3><span className="text-xs text-[var(--color-text-muted)]">{docs.length} 个</span></div>
            {loadingDocs ? <div className="p-8 text-center text-sm text-[var(--color-text-muted)]">加载中...</div>
            : docs.length === 0 ? <div className="p-8 text-center text-sm text-[var(--color-text-muted)]">暂无上传文件</div>
            : <div className="divide-y divide-[var(--color-border)]">{docs.map(d => (
              <div key={d.id} className="flex items-center gap-4 px-5 py-3.5 hover:bg-gray-50">
                <span className="text-2xl">{fileIcon(d.name)}</span>
                <div className="flex-1 min-w-0">
                  <div className="text-sm font-medium truncate">{d.name}</div>
                  <div className="text-xs text-[var(--color-text-muted)] flex gap-3 mt-0.5">
                    <span>{d.size > 1024 * 1024 ? (d.size / (1024 * 1024)).toFixed(1) + " MB" : (d.size / 1024).toFixed(1) + " KB"}</span>
                    <span>{new Date(d.created_at).toLocaleDateString("zh-CN")}</span>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <a href={d.url || ("https://ai-course-assistant.oss-cn-beijing.aliyuncs.com/" + d.r2_key)} target="_blank" rel="noopener noreferrer" className="text-xs text-[var(--color-primary)] hover:underline">查看</a>
                  <span className="text-[10px] px-2 py-0.5 rounded-full font-medium bg-green-100 text-green-700">已就绪</span>
                  <button onClick={() => handleDelete(d)} className="text-[var(--color-text-muted)] hover:text-red-500">🗑</button>
                </div>
              </div>
            ))}</div>}
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-white rounded-xl border border-[var(--color-border)] p-6">
              ...
              <div className="grid grid-cols-2 gap-4">
                {[["文档总数", docs.length, "bg-blue-50 text-blue-700"], ["向量分块", 0, "bg-green-50 text-green-700"], ["章节数", 0, "bg-purple-50 text-purple-700"], ["引用页数", 0, "bg-amber-50 text-amber-700"]].map(([l, v, c]) => (
                  <div key={String(l)} className={c + " rounded-lg p-4 text-center"}><div className="text-2xl font-bold">{String(v)}</div><div className="text-xs opacity-70 mt-1">{String(l)}</div></div>
                ))}
              </div>
            </div>
            <div className="bg-white rounded-xl border border-[var(--color-border)] p-6">
              <h3 className="text-sm font-bold mb-4">📑 文档列表</h3>
              {docs.length === 0 ? <div className="text-center text-sm text-[var(--color-text-muted)] py-8">上传文件后显示</div>
              : <div className="space-y-3">{docs.map(d => (
                <div key={d.id} className="flex items-center justify-between bg-gray-50 rounded-lg p-3"><div className="flex items-center gap-3"><span>{fileIcon(d.name)}</span><div className="text-xs font-medium truncate max-w-[200px]">{d.name}</div></div><span className="text-[10px] text-[var(--color-text-muted)]">待向量化</span></div>
              ))}</div>}
            </div>
          </div>
      ...
      {activeTab === "students" && (
        <div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            {[["👥", mockStudents.length, "学生总数"], ["💬", mockStudents.reduce((s, st) => s + st.knowledgeQueries, 0), "知识查询"], ["✅", mockStudents.reduce((s, st) => s + st.guidedCompleted, 0), "完成引导"], ["🗺️", mockStudents.reduce((s, st) => s + st.sandboxSessions, 0), "沙盘实验"]].map(([i, v, l]) => (
              <div key={String(l)} className="bg-white rounded-xl border border-[var(--color-border)] p-4"><div className="text-2xl mb-1">{i}</div><div className="text-2xl font-bold">{String(v)}</div><div className="text-xs text-[var(--color-text-muted)]">{String(l)}</div></div>
            ))}
          </div>
          <div className="bg-white rounded-xl border border-[var(--color-border)] overflow-hidden">
            <div className="px-5 py-3 border-b"><h3 className="text-sm font-bold">学生详情</h3></div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm"><thead><tr className="border-b bg-gray-50">{["姓名","总访问","知识查询","完成引导","沙盘实验","最近活跃"].map(h => <th key={h} className="px-3 py-3 text-xs font-medium text-[var(--color-text-secondary)] text-center">{h}</th>)}</tr></thead><tbody>
