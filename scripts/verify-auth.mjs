...
async function main() {
  ...
  // ── E. 过期验证码 ──
  try {
    const r0 = await sendCode(EMAIL, "RESET_PASSWORD");
    await pool.query("UPDATE verification_codes SET expires_at = now() - interval '1 minute' WHERE identifier = $1 AND purpose = 'RESET_PASSWORD' AND consumed_at IS NULL", [EMAIL]);
    const r = await post("/api/auth/verification/verify", { identifier: EMAIL, purpose: "RESET_PASSWORD", code: r0.data.echoCode });
    record("E 过期验证码拒绝", r.status === 400);
    await pool.query("DELETE FROM verification_codes WHERE identifier = $1 AND purpose = 'RESET_PASSWORD'", [EMAIL]);
  ...
  try {
    ...
    const wrong = "000000";
    const r = await post("/api/auth/verification/verify", { identifier: EMAIL, purpose: "RESET_PASSWORD", code: wrong });
  ...
  try {
    const r0 = await sendCode(EMAIL, "RESET_PASSWORD");
    const r1 = await post("/api/auth/verification/verify", { identifier: EMAIL, purpose: "RESET_PASSWORD", code: r0.data.echoCode });
    const firstOk = r1.status === 200;
    const r2 = await post("/api/auth/verification/verify", { identifier: EMAIL, purpose: "RESET_PASSWORD", code: r0.data.echoCode });
  ...
  // ── I. 不存在账号找回密码：防枚举 ──
  try {
    const ghost = `ghost-${ts}@demo.test`;
    const rGhost = await sendCode(ghost, "RESET_PASSWORD");
    const rReal = await sendCode(EMAIL, "RESET_PASSWORD");
    ...
    await pool.query("DELETE FROM verification_codes WHERE identifier = ANY($1)", [[ghost, EMAIL]]);
